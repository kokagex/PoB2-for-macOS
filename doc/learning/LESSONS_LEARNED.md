...
## ⚠️ サブエージェントによる機能コード誤削除（失敗→成功）

### 診断ログクリーンアップで ProcessControlsInput が誤削除された（失敗）
...
**状況**:
- Phase I 診断ログクリーンアップをサブエージェント3並列で実行
- Build.lua のクリーンアップエージェントが、診断コメント `-- (DIAG-SEL removed)` と同じ行付近にあった `self:ProcessControlsInput(inputEvents, main.viewPort)` を誤って削除
- 結果: Options ボタン以外の全ボタンが反応しなくなった（描画はされるが入力が処理されない）

**原因**:
- サブエージェントが「DIAG-SEL」という診断用コメントを削除対象と判断した際に、隣接する `ProcessControlsInput` 行も一緒に削除した
- `ProcessControlsInput` は `DrawControls` と対をなす重要な関数で、UI コントロールのマウス/キーボード入力を処理する
- `DrawControls` = 描画のみ、`ProcessControlsInput` = 入力処理 → 片方がないとボタンは見えるが反応しない

**検出方法**:
- 視覚的検証でユーザーが「ボタンが反応しない」と報告
- バックアップファイル（Build.lua.backup_phase_h）と差分比較で発見
- `grep ProcessControlsInput` でバックアップにはline 1860に存在、現ファイルには不在
...
**教訓**:
1. **サブエージェントの出力は必ず検証**: サブエージェントは指示に従うが、隣接する機能コードとの境界を誤認する可能性がある
2. **クリーンアップ後は機能テスト必須**: ログ削除は「安全」に見えるが、コメントと機能コードが近接している場合に巻き添え削除が起きる
3. **バックアップとの diff 比較**: クリーンアップ後に `grep` で重要な関数名（ProcessControlsInput, DrawControls, OnFrame 等）が存在するか確認すべき
4. **視覚的検証の価値**: ログだけでは検出不可能な問題を、ユーザーのクリック操作で即座に発見できた

**予防策**:
- サブエージェントにクリーンアップを依頼する際は、「削除してはいけない関数リスト」を明示する
- クリーンアップ後に `grep "ProcessControlsInput\|DrawControls\|OnFrame\|Init"` で重要関数の存在を確認
- 機能テスト（ボタンクリック、タブ切替、ノード割当）を必ず実施

**適用**:
- サブエージェントへの大規模コード変更依頼時は、変更後の機能検証を必須化
- 「ログ削除のみ」でも、隣接コードの巻き添え削除リスクを認識する
...
### 42. ルーティン最適化: ユーザー・ファースト原則 (2026-02-08)

**カテゴリ**: 効率化・プロセス改善
**発生**: S/Aフィルタボタン修正セッション
**影響**: 中（8-10ラウンドのコンテキスト浪費）

**問題**:
- 視覚テスト失敗後、ユーザーに質問する前に8-10ラウンドの自律コード分析を実施
- Metal backend, Tooltip, EditControl, viewport等を網羅的に調査
- **全て不要だった** — 「ツールチップは見えますか？」の1問で問題を特定

**教訓**:
1. ユーザーの1回の観察 > 10ラウンドのコード分析
2. 自律分析は最大2ラウンド、その後必ずユーザーに質問
3. プランの根本原因は「仮説」であり、視覚テスト失敗で即座に疑う
4. テスト指示は番号付きステップ + 「やらないこと」を明記

**改善実施**: CLAUDE.mdのError Handlingセクションを5ステッププロトコルに改訂
- 即時質問（2ラウンド以内）
- 3候補予測（プラン前提の誤りも候補に含む）
- 明確なテスト指示テンプレート
- コンテキスト予算の明示的制限
- 禁止事項の明確化
...
### 43. 無関係行動・根拠なき疑い（失敗） (2026-02-10)

**カテゴリ**: 失敗パターン / コミュニケーション
**発生**: ツールチップ不具合調査セッション
**影響**: 高（信頼低下・時間浪費）

**状況**:
- 本来の依頼と直接関係のない行動を実行した
- 根拠のない推測でユーザー側の問題を疑った
...
**最終更新**: 2026-02-10 (無関係行動・根拠なき疑いの失敗を追加)
**総学習記録数**: 43件 (成功15件、失敗13件、繰り返し3件、効率化3件、技術的発見7件、エージェントシステム改善1件、診断失敗2件、プロジェクト失敗1件)
**次回更新**: 新しい学習発生時、即座に
