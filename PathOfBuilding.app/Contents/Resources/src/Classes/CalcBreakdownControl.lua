		...
		elseif section.type == "TABLE" then
			...
			for _, col in pairs(section.colList) do
				for _, row in pairs(section.rowList) do
					if row[col.key] then
						local cellVal = tostring(row[col.key])
						local _, num = string.gsub(cellVal, "%d%d%d%d", "") -- count how many commas will be added
						if main.showThousandsSeparators and num > 0 then
							col.width = m_max(col.width or 0, DrawStringWidth(16, "VAR", col.label) + 6, DrawStringWidth(12, "VAR", cellVal) + 6 + (4 * num))
						else
							col.width = m_max(col.width or 0, DrawStringWidth(16, "VAR", col.label) + 6, DrawStringWidth(12, "VAR", cellVal) + 6)
						end
					end
				end
				if col.width then
...
function CalcBreakdownClass:AddModSection(sectionData, modList)
	...
	local cfg = (sectionData.cfg and actor.mainSkill and actor.mainSkill[sectionData.cfg.."Cfg"] and copyTable(actor.mainSkill[sectionData.cfg.."Cfg"], true)) or { }
...
function CalcBreakdownClass:Draw(viewPort)
	...
	if not sourceData or not self.contentHeight then return end
