...
static bool sg_parse_escape_code(const char** text, float* color) {
    ...
    if (*s >= '0' && *s <= '9') {
        ...
        static const float colors[10][3] = {
            ...
            {1.0f, 1.0f, 1.0f},  // ^7 White (default text)
...
static const char* sg_get_font_path(const char* font_name) {
    ...
    if (strlen(font_name) < 3) {
        fprintf(stderr, "Warning: Invalid font name '%s', using Helvetica\n", font_name);
        return helvetica_path;
    }
...
static SGFontFace* sg_load_font(SGContext* ctx, const char* font_name, int size) {
    ...
    int scaled_size = (int)(size * 0.85f);
    if (scaled_size < 1) scaled_size = 1;
    if (FT_Set_Pixel_Sizes(ft_face, 0, scaled_size)) {
...
void DrawString(int left, int top, int align, int height,
    ...
    // Apply ascender offset so text baseline aligns properly
    FT_Size_Metrics metrics = ft_face->size->metrics;
    int ascender = (int)(metrics.ascender >> 6);
    int y = top + ascender;
