...
function CalcsTabClass:CalcsTab(build)
	...
	self.controls.search = new("EditControl"):EditControl({ "TOPLEFT", self, "TOPLEFT" }, { 4, 5, 260, 20 }, "", i18n.lookup("calcs", "Search"), "%c", 100, nil, nil, nil, true)
...
function CalcsTabClass:CheckFlag(obj, actor, player)
	...
	local activeEffect = actor.mainSkill.activeEffect or {}
	local skillFlags = (activeEffect.statSetCalcs or activeEffect.statSet or {}).skillFlags or actor.mainSkill.skillFlags or {}
	...
	local playerActor = player or self.calcsEnv.player
	local playerActiveEffect = playerActor and playerActor.mainSkill and playerActor.mainSkill.activeEffect or {}
	local playerSkillFlags = (playerActiveEffect.statSetCalcs or playerActiveEffect.statSet or {}).skillFlags or (playerActor and playerActor.mainSkill and playerActor.mainSkill.skillFlags) or {}
...
function CalcsTabClass:GetMiscCalculator()
	if not self.miscCalculator then
		return nil, nil
	end
