...
function ConfigTabClass:ConfigTab(build)
	...
	self.controls.setLabel = new("LabelControl"):LabelControl({ "RIGHT", self.controls.setSelect, "LEFT" }, { -2, 0, 0, 16 }, "^7" .. i18n.t("config.ui.configSet"))
	self.controls.setManage = new("ButtonControl"):ButtonControl({ "LEFT", self.controls.setSelect, "RIGHT" }, { 4, 0, 90, 20 }, i18n.t("config.ui.manage"), function()
		self:OpenConfigSetManagePopup()
	end)

	self.controls.search = new("EditControl"):EditControl({ "TOPLEFT", self.controls.sectionAnchor, "TOPLEFT" }, { 8, 15, 360, 20 }, "", i18n.t("config.ui.search"), "%c", 100, function()
		self:UpdateControls()
	end, nil, nil, true)
	...
	for _, varData in ipairs(varList) do
		if varData.section then
			lastSection = new("SectionControl"):SectionControl({ "TOPLEFT", self.controls.search, "BOTTOMLEFT" }, { 0, 0, 360, 0 }, translateSection(varData.section))
			...
			if varData.type == "check" then
				control = new("CheckBoxControl"):CheckBoxControl({ "TOPLEFT", lastSection, "TOPLEFT" }, { 234, 0, 18 }, translateLabel(varData), function(state)
			...
			if varData.label and varData.type ~= "check" then
				...
				labelControl = new("LabelControl"):LabelControl({ "RIGHT", control, "LEFT" }, { -4, 0, 0, DrawStringWidth(14, "VAR", displayLabel) > 228 and 12 or 14 }, "^7" .. displayLabel)
