...
function CalcSectionClass:DrawContent(drawX, startLineY, drawWidth, actor, viewPort, isOverlay, noTooltip)
	...
	for _, subSec in ipairs(self.subSection) do
		...
		-- Label
		local displayLabel = i18n.lookup("calcs", subSec.label) or subSec.label or ""
		if not enabled then
			local lx = isOverlay and drawX + 4 or drawX + 3
			DrawString(lx, lineY + 3, "LEFT", 16, "VAR BOLD", "^8"..displayLabel)
		else
			local lx = isOverlay and drawX + 4 or drawX + 3
			local textColor = "^7"
			if not isOverlay and (self.calcsTab:SearchMatch(subSec.label) or self.calcsTab:SearchMatch(displayLabel)) then
				textColor = colorCodes.HIGHLIGHT
			end
			DrawString(lx, lineY + 3, "LEFT", 16, "VAR BOLD", textColor..displayLabel..":")
			if subSec.data.extra then
				local ex = lx + DrawStringWidth(16, "VAR BOLD", displayLabel) + (isOverlay and 12 or 10)
				...
				if (isOverlay and actor and self.calcsTab:CheckFlag(rowData)) or (not isOverlay and rowData.enabled) then
					...
					if not isOverlay then
						...
						if rowData.label then
							...
							DrawString(drawX + 132, lineY + 2, "RIGHT_X", 16, "VAR", textColor..displayRowLabel.."^7:")
						end
					elseif rowData.label then
						DrawString(drawX + 132, lineY + 2, "RIGHT_X", 16, "VAR", "^7"..(i18n.lookup("calcs", rowData.label) or rowData.label).."^7:")
					end
					for _, colData in ipairs(rowData) do
